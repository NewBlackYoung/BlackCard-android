...
public class AllAdapter extends RecyclerView.Adapter<AllAdapter.ViewHolder> implements OnNetResultListener {
    private Context context;
    private List<NewFindBean.DataBean.AllfindBean> list;

    public void setList(List<NewFindBean.DataBean.AllfindBean> list) {
        this.list = list;
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            ll_like = (LinearLayout) itemView.findViewById(R.id.ll_like); //  喜欢数量
    ...
    private void photo(ViewHolder viewHolder, final int i) {
        ...
        if (list.get(i).getImage_urls() != null && list.get(i).getImage_urls().size() >= 1 && !list.get(i).getImage_urls().get(0).getImg().equals("")) {
            ...
            viewHolder.rv_photo.setVisibility(View.VISIBLE);
            viewHolder.rv_photo.setAdapter(p);
        } else {
            viewHolder.rv_photo.setVisibility(View.GONE);
        }
    ...
    private void checkPandF(ViewHolder viewHolder, int i) {
        if (list.get(i).getTopic() != null) { //判断是否有话题
            viewHolder.tv_smamubiao.setVisibility(View.VISIBLE);
            viewHolder.tv_smamubiao.setText(list.get(i).getTopic().toString());
        } else {
            viewHolder.tv_smamubiao.setVisibility(View.GONE);
        }
