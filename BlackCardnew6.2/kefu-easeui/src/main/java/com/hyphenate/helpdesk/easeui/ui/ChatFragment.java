 ...
 */
public class ChatFragment extends BaseFragment implements ChatManager.MessageListener, EmojiconManager.EmojiconManagerDelegate, OnClickListener {
    ...
    private String name;
    private RelativeLayout lay_jianjieyu;
    private TextView tv_dingjiudian;
    private TextView tv_lookorder;
    private TextView tv_lookhangban;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        stringTalk = getActivity().getIntent().getStringExtra("talk");
    ...
    protected void initView() {
        // 头像
        tv_tousu = (TextView) getView().findViewById(R.id.tv_tousu);
        tv_tousu.setOnClickListener(this);
        tv_image_title = (TextView) getView().findViewById(R.id.iv_image_title);// 头像
        // 间接语
        lay_jianjieyu = (RelativeLayout) getView().findViewById(R.id.lay_jianjieyu);
        lay_jianjieyu.setOnClickListener(this);
        // 订酒店
        tv_dingjiudian = (TextView) getView().findViewById(R.id.tv_dingjiudian);
        tv_dingjiudian.setOnClickListener(this);
        // 查看订单
        tv_lookorder = (TextView) getView().findViewById(R.id.tv_lookorder);
        tv_lookorder.setOnClickListener(this);
        // 查看航班
        tv_lookhangban = (TextView) getView().findViewById(R.id.tv_lookhangban);
        tv_lookhangban.setOnClickListener(this);
        SpUtil spUtil = new SpUtil(getContext(), "name_kf");
        String fu = spUtil.getSharedPreference("kf_name", "").toString();
        number = spUtil.getSharedPreference("kf_number", "").toString();
        if (fu.equals("")) {
            tv_image_title.setText("青年黑卡");
        } else {
            tv_image_title.setText(fu);
        }
    ...
    protected void setUpView() {
        ...
        if (content != null && title != null && !content.equals("") && !title.equals("")) {
            sendTextMessage(title + content);
        }
        if (type != null && !type.equals("")) {
            if (type.equals("1")) {
                sendTextMessage("我要订机票");
            }
            if (type.equals("2")) {
                sendTextMessage("我要订酒店");
            }
            if (type.equals("3")) {
                sendTextMessage("我要开通云逸会籍");
            }
            if (type.equals("4")) {
                sendTextMessage(stringTalk);
            }
            if (type.equals("5")){
                sendTextMessage("我在任务中心遇到问题");
            }
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAMERA) { // 发送照片
                ...
                //ToastUtils.show(getContext(), "常用语或者打赏");
    ...
    public void onClick(View v) {
        int i = v.getId();
        if (i == R.id.tv_lookhangban) {
            sendTextMessage("请帮我查下航班");
            // 查看航班
        }
        if (i == R.id.tv_dingjiudian) {
            // 订酒店
            sendTextMessage("请帮我订酒店");
        }
        if (i == R.id.tv_lookorder) {
            // 查看订单
            sendTextMessage("请问好物订单如何查询");
        }
        if (i == R.id.lay_jianjieyu) {
        ...
        lay_jianjieyu.setVisibility(View.GONE);
    }
        ...
        public void onExtendMenuItemClick(int itemId, View view) {
            ...
            switch (itemId) {
                ...
                case ITEM_PICTURE:
                    ...
                    break;
                case ITEM_VIDEO:
                    lay_jianjieyu.setVisibility(View.VISIBLE);
                    // ToastUtils.show(getContext(), "常用语");
                    break;
                case ITEM_FILE:
                    if (number == null || number.equals("")) {
                        ToastUtils.show(getContext(), "获取客服信息失败！无法进行打赏");
                        return;
                    }
                    Intent intent = new Intent();
                    intent.setAction("com.DaShangActivity.action");
                    intent.addCategory("android.intent.category.DEFAULT");
    ...
    public void onMessage(List<Message> msgs) {
        ...
        AgentInfo agentInfo = MessageHelper.getAgentInfo(msgs.get(0));
        name = agentInfo.getNickname();
        String info = agentInfo.getContent().toString();
        try {
            JSONObject object = new JSONObject(info);
            number = object.getString("agentNumber");//客服编号
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(android.os.Message msg) {
            tv_image_title.setText(name);
            SpUtil spUtil = new SpUtil(getContext(), "name_kf");
            spUtil.put("kf_name", name);
            spUtil.put("kf_number", number);
        }
    };
